...
## Individual exercises - answers (1) 
- histogram of weta abundance (total.weta) with black outline and grey fill

```{r, eval = FALSE}
ggplot(weta_data, aes(x = total.weta)) +
  geom_histogram(colour = "black",
                 fill = "grey") 

```

## Individual exercises - answers (2) 
- scatter plot of NCOR against substrate density, colour by topo, size by shrub, theme_classic

```{r, eval = FALSE}
ggplot(mite_all, aes(x = SubsDens, y = NCOR)) +
...
Assuming you have completed all assumption checks, overdispersion, etc (the model we will use is in fact overdispersed).
...
## Saving
Just discussed in workshop as an extra. See the extra materials folder, ggplot2 intermediate handout for code and discussion.
...
Or come to the poster session on Tuesday :) 
